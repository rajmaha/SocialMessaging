# Todos Sidebar Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Extract todos logic into a shared `TodosPanel` component rendered both on the full `/reminders` page and in a slide-in right sidebar toggled from the header icon.

**Architecture:** `TodosPanel.tsx` holds all state/logic/modals from the current reminders page. `reminders/page.tsx` becomes a thin wrapper. `TodosSidebar.tsx` is a fixed drawer rendering `<TodosPanel mode="sidebar" />`. The header icon button (not a Link) toggles the sidebar open/closed.

**Tech Stack:** Next.js 14 App Router, TypeScript, TailwindCSS, react-icons/fi. No test framework — verify manually in browser at http://localhost:3000.

**Note:** No Alembic, no Jest. Backend unchanged. Frontend only.

---

### Task 1: Create `TodosPanel.tsx`

**Files:**
- Create: `frontend/components/TodosPanel.tsx`
- Reference: `frontend/app/reminders/page.tsx` (copy logic from here)

**Step 1: Create the file**

Create `frontend/components/TodosPanel.tsx` with the following content. This is the full todos logic extracted from `reminders/page.tsx`, accepting a `mode` prop:
...
export default function TodosPanel({ mode = 'page' }: TodosPanelProps) {
    ...
    return (
        <div className={isSidebar ? 'flex flex-col h-full' : 'max-w-5xl mx-auto p-6'}>
            {/* Header */}
            <div className={`flex items-center justify-between mb-6 ${isSidebar ? 'px-4 pt-4' : ''}`}>
                <h1 className={`font-bold text-gray-800 ${isSidebar ? 'text-lg' : 'text-2xl'}`}>My Todos</h1>
                <button
            ...
            {/* Tabs + Filters */}
            <div className={`flex items-center justify-between mb-4 ${isSidebar ? 'px-4' : ''}`}>
                <div className="flex gap-1 bg-gray-100 p-1 rounded-lg">
                    <button
                        onClick={() => setActiveTab('my')}
                        className={`px-3 py-1.5 rounded-md text-sm font-medium transition ${activeTab === 'my' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                        My Todos
                    </button>
                    <button
                        onClick={() => { setActiveTab('shared'); fetchSharedReminders() }}
                        className={`px-3 py-1.5 rounded-md text-sm font-medium transition relative ${activeTab === 'shared' ? 'bg-white shadow text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                        Shared With Me
            ...
            <div className={isSidebar ? 'flex-1 overflow-y-auto px-4 pb-4' : ''}>
                {/* My Todos Tab */}
                {activeTab === 'my' && (
                    <>
                        {loading ? (
                                        ...
                                        <div className="flex items-start justify-between">
                                            <div className="flex-1" onClick={() => openDetail(r)}>
                                                <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                    <h3 className={`font-semibold text-gray-800 text-sm ${r.status === 'completed' ? 'line-through text-gray-400' : ''}`}>
                                                        {r.title}
                                                    </h3>
                                                ...
                                                <div className="flex items-center gap-3 text-xs text-gray-400">
                                                    {r.due_date && (
                                                        <span className="flex items-center gap-1">
                                                            <FiCalendar size={11} /> {formatDate(r.due_date)}
                                                        </span>
                                                    )}
                                                    {r.share_count > 0 && (
                                                        <span className="flex items-center gap-1">
                                                            <FiShare2 size={11} /> {r.share_count}
                                                        </span>
                                                    )}
                                                    {r.comment_count > 0 && (
                                                        <span className="flex items-center gap-1">
                                                            <FiMessageSquare size={11} /> {r.comment_count}
                                                        </span>
                                                    )}
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-1 ml-2">
                                                ...
                                                <div className="relative">
                                                    <button onClick={() => setShowSocialShare(showSocialShare === r.id ? null : r.id)} title="Social share"
                                                        className="p-1 text-indigo-500 hover:bg-indigo-50 rounded text-xs font-bold">
                                                        S
                                                    </button>
                            ...
                            sharedReminders.map(s => (
                                ...
                                >
                                    <div className="flex items-start justify-between">
                                        <div>
                                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                                                {!s.is_seen && <span className="h-2 w-2 rounded-full bg-blue-500 flex-shrink-0" />}
                                                <h3 className="font-semibold text-gray-800 text-sm">{s.reminder_title}</h3>
                                                {s.reminder_priority && (
                                            ...
                                            {s.reminder_description && <p className="text-xs text-gray-500 mb-1">{s.reminder_description}</p>}
                                            <div className="text-xs text-gray-400 flex items-center gap-3">
                                                <span>Shared by <strong>{s.sharer_name}</strong></span>
                                                {s.reminder_due_date && (
                                                    <span className="flex items-center gap-1"><FiCalendar size={11} /> {formatDate(s.reminder_due_date)}</span>
                                                )}
                                            </div>
                                        </div>
                                    </div>
            ...
            {/* Edit Modal */}
            {editingReminder && (
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[100]" onClick={() => setEditingReminder(null)}>
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            ...
            {/* Reschedule Modal */}
            {showRescheduleModal && (
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[100]" onClick={() => setShowRescheduleModal(false)}>
            ...
            {showDetailModal && selectedReminder && (
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[100]" onClick={() => { setShowDetailModal(false); setSelectedReminder(null) }}>
...
```bash
ls frontend/components/TodosPanel.tsx
```
...
```bash
git add frontend/components/TodosPanel.tsx
git commit -m "feat: extract TodosPanel shared component from reminders page"
```

---

### Task 2: Simplify `reminders/page.tsx` to use `TodosPanel`

**Files:**
- Modify: `frontend/app/reminders/page.tsx`
...
Navigate to http://localhost:3000/reminders. The page should look and work identically to before.

**Step 3: Commit**

```bash
git add frontend/app/reminders/page.tsx
git commit -m "refactor: reminders page uses shared TodosPanel component"
```

---

### Task 3: Create `TodosSidebar.tsx`

**Files:**
- Create: `frontend/components/TodosSidebar.tsx`
...
```bash
git add frontend/components/TodosSidebar.tsx
git commit -m "feat: add TodosSidebar slide-in drawer component"
```

---

### Task 4: Wire sidebar toggle into `MainHeader.tsx`
...
At the top of the file, add the import for `TodosSidebar` and `useState` (useState is already imported, just add the component):
...
Inside `MainHeaderInner`, after the existing `const [isMounted, setIsMounted] = useState(false)` line, add:

```tsx
const [todosSidebarOpen, setTodosSidebarOpen] = useState(false)
```

**Step 3: Add right-side icon button**

In the JSX, find the right-side section:
```tsx
...
**Step 4: Render `TodosSidebar` in the return**

Just before the closing `</header>` tag, add:
...
1. Any page — click the `FiCheckSquare` icon in the header right side → sidebar slides in from the right with full todos functionality
2. Click backdrop or X button → sidebar closes
3. Press Escape → sidebar closes
4. Click "My Todos" nav tab → navigates to `/reminders` full page as before

**Step 6: Commit**

```bash
git add frontend/components/MainHeader.tsx
git commit -m "feat: add todos sidebar toggle to header icon"
```
