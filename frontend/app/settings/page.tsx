...
function SettingsPage() {
  ...
  const [providerAvailability, setProviderAvailability] = useState<{ google_enabled: boolean; microsoft_enabled: boolean }>({ google_enabled: false, microsoft_enabled: false })

  const fetchProviderAvailability = async () => {
    try {
      const token = getAuthToken()
      if (!token) return
      const resp = await axios.get(`${API_URL}/api/calendar/settings/status`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      setProviderAvailability(resp.data)
    } catch (e) {
      console.error('Failed to load provider availability', e)
    }
  }
  ...
  useEffect(() => {
    ...
    if (activeTab === 'calendar') { fetchCalendarStatus(); fetchProviderAvailability() }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeTab])
          ...
          <div className="p-6">
            ...
            {activeTab === 'calendar' && (
              <div>
                ...
                {!providerAvailability.google_enabled && !providerAvailability.microsoft_enabled ? (
                  <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 text-center text-gray-500">
                    <p className="font-medium mb-1">Calendar integration is not configured</p>
                    <p className="text-sm">Please contact your administrator to enable calendar sync.</p>
                  </div>
                ) : (
                  <>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {/* Google Calendar */}
                      {providerAvailability.google_enabled && (
                        <div className="border border-gray-200 rounded-lg p-6">
                          <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-500">Sync todos with Google Calendar</p>
                            </div>
                          </div>
                      ...
                      {/* Microsoft Calendar */}
                      {providerAvailability.microsoft_enabled && (
                        <div className="border border-gray-200 rounded-lg p-6">
                          <div className="flex items-center gap-3 mb-4">
                            <div className="w-10 h-10 bg-white border border-gray-200 rounded-lg flex items-center justify-center">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-500">Sync todos with Outlook Calendar</p>
                            </div>
                          </div>
                    ...
                    <div className="mt-6 bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-900">
                      ...
                      <ul className="list-disc list-inside space-y-1 text-blue-800">
                        <li>New todos with a due date are automatically added to your calendar</li>
                        <li>Rescheduling a todo updates the calendar event</li>
                        <li>Completing a todo removes the calendar event</li>
