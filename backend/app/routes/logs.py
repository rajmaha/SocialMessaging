...
def _error_row(r: ErrorLog) -> dict:
    return {
        "id": r.id,
        "timestamp": r.timestamp.strftime("%Y-%m-%dT%H:%M:%S") if r.timestamp else None,
        "severity": r.severity,
...
def list_audit_logs(
    ...
    return {"total": total, "page": page, "page_size": page_size, "items": [_audit_row(r) for r in items]}
...
def get_audit_log(
    ...
    r = log_db.query(AuditLog).filter(AuditLog.id == log_id).first()
    if not r:
...
def list_error_logs(
    ...
    return {"total": total, "page": page, "page_size": page_size, "items": [_error_row(r) for r in items]}
...
@router.get("/errors/{log_id}")
def get_error_log(
    log_id: int,
    log_db: Session = Depends(get_log_db),
    _admin=Depends(get_admin_user),
):
    r = log_db.query(ErrorLog).filter(ErrorLog.id == log_id).first()
    if not r:
        raise HTTPException(status_code=404, detail="Error log entry not found")
    return _error_row(r)


# ── Frontend error capture (unauthenticated) ──────────────────────────────────
