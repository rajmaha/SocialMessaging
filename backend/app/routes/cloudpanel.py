...
def create_server(server: CloudPanelServerCreate, db: Session = Depends(get_db)):
...
def create_site_on_server(server_id: int, site_data: CloudPanelSiteCreate, db: Session = Depends(get_db)):
...
def get_ssl_report(server_id: int, db: Session = Depends(get_db)):
    server = db.query(CloudPanelServer).filter(CloudPanelServer.id == server_id).first()
    if not server:
        raise HTTPException(status_code=404, detail="Server not found")
...
def renew_ssl(server_id: int, domain: str, db: Session = Depends(get_db)):
    server = db.query(CloudPanelServer).filter(CloudPanelServer.id == server_id).first()
    if not server:
        raise HTTPException(status_code=404, detail="Server not found")
