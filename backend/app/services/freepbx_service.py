...
class FreePBXService:
    ...
    def _configure_webrtc_via_ssh(self, db, extension: str) -> Tuple[bool, str]:
        """Configure WebRTC PJSIP settings via SSH using a 3-layer approach:

        1. Update the `sip` table (FreePBX UI source of truth) so the admin panel
           shows correct values for DTLS, ICE, codecs, etc.
        2. Write to /etc/asterisk/pjsip.endpoint_custom_post.conf using [EXT](+)
           syntax to guarantee Asterisk loads WebRTC settings — even if fwconsole
           reload's genConfig produces an incomplete endpoint section.
        3. Run fwconsole reload + asterisk core reload.
        """
        ssh_cfg, err = self._get_ssh_settings(db)
        if not ssh_cfg:
            return False, err

        # --- Layer 1: sip table (for FreePBX admin UI) ---
        sip_rows = [
        ...
        sip_sql = f"INSERT INTO sip (id, keyword, data, flags) VALUES {values_sql} ON DUPLICATE KEY UPDATE data=VALUES(data);"

        # --- Layer 2: pjsip.endpoint_custom_post.conf (for Asterisk) ---
        # [EXT](+) adds settings to the existing endpoint section generated by fwconsole.
        # `webrtc=yes` is an Asterisk master switch that auto-enables DTLS, ICE, AVPF, etc.
        custom_conf_block = (
            f"\\n[{extension}](+)\\n"
            f"webrtc=yes\\n"
            f"dtls_auto_generate_cert=yes\\n"
            f"media_encryption=dtls\\n"
            f"ice_support=yes\\n"
            f"bundle=yes\\n"
            f"media_use_received_transport=yes\\n"
            f"rtp_symmetric=yes\\n"
            f"force_rport=yes\\n"
            f"rewrite_contact=yes\\n"
            f"trust_id_inbound=yes\\n"
        )
        custom_conf_path = "/etc/asterisk/pjsip.endpoint_custom_post.conf"
        ...
        try:
            ...
            if exit_code != 0:
                logger.warning("SSH: sip table write failed (exit %d): %s", exit_code, err_output)
            else:
                logger.info("✅ SSH: wrote %d WebRTC settings to sip table for extension %s", len(sip_rows), extension)

            # Step 2: Update pjsip.endpoint_custom_post.conf
            # Remove any existing block for this extension, then append the new one.
            remove_cmd = (
                f"sed -i '/^\\[{extension}\\](+)/,/^\\[/{{/^\\[{extension}\\](+)/d;/^\\[/!d;}}' "
                f"{custom_conf_path} 2>/dev/null; "
                f"sed -i '/^$/N;/^\\n$/d' {custom_conf_path} 2>/dev/null"
            )
            stdin, stdout, stderr = client.exec_command(remove_cmd, timeout=10)
            stdout.channel.recv_exit_status()

            append_cmd = f'printf "{custom_conf_block}" >> {custom_conf_path}'
            stdin, stdout, stderr = client.exec_command(append_cmd, timeout=10)
            exit_code = stdout.channel.recv_exit_status()

            if exit_code != 0:
                err_output = stderr.read().decode(errors="ignore").strip()
                logger.warning("SSH: custom_post.conf write failed (exit %d): %s", exit_code, err_output)
            else:
                logger.info("✅ SSH: wrote WebRTC block to %s for extension %s", custom_conf_path, extension)

            # Step 3: fwconsole reload (regenerates base config) + core reload (picks up custom_post)
            reload_cmds = [
            ...
            reload_results = []
            for cmd, label in reload_cmds:
                logger.info("SSH: running %s …", label)
                ...
                if r_exit == 0:
                    ...
                    reload_results.append(f"{label}: OK")
                else:
                    logger.warning("SSH: %s exit %d: %s %s", label, r_exit, r_out, r_err)
                    reload_results.append(f"{label}: exit {r_exit}")

            client.close()
            return True, f"WebRTC configured ({len(sip_rows)} sip settings + custom_post.conf). {', '.join(reload_results)}"
